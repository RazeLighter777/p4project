...
- **Start learning bridge (Python controller) for each switch**.
  - Open a new terminal and connect to `s1` on port 50001
    ```bash
    $ make controller name=bridge grpc_port=50001 topo=linear,2,2
    ```
  - Open another terminal and connect to `s2` on port 50002 
    ```bash
    $ make controller name=bridge grpc_port=50002 topo=linear,2,2
    ```
...
> **INFO:** The reference solution has roughly **40** and **30** (well commented and spaced) lines of code in the seven total `TODO` sections of the `switch.p4` code and `switch.py` script, respectively.
...
- **Start learning switch (Python controller) for each switch**.
  - Open a new terminal and connect to `s1` on port 50001
    ```bash
    $ make controller name=switch grpc_port=50001 topo=linear,2,2
    ```
  - Open another terminal and connect to `s2` on port 50002 
    ```bash
    $ make controller name=switch grpc_port=50002 topo=linear,2,2
    ```
...
- **Let's ping `h2s2` from `h1s1` sitting across the two switches (with VLAN)**.
  ...
  ```bash
  $ cd final/project
  $ make enable-vlan topo=linear,2,2
  ```
  > **INFO:** You can disable VLAN anytime by running `make disable-vlan topo=linear,2,2`.
